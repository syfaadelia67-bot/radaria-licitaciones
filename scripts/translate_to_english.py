...
def request_translations(records: list[dict[str, Any]], token: str, model: str, timeout: int = 90) -> list[dict[str, Any]]:
    compact = []
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "TenderSignal-English-Layer/1.0",
        },
        method="POST",
    )
...
def apply_entry(record: dict[str, Any], entry: dict[str, Any], model: str) -> None:
    ...
    record["translation"] = {
        ...
        "translated_at": clean_text(entry.get("translated_at")) or datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
    }
...
def mark_original_only(record: dict[str, Any]) -> None:
    title, description = original_text(record)
    record["title"] = title
    record["description"] = description
    record.pop("original_title", None)
...
def translate_records(
    ...
    stats = {"source_english": 0, "cached": 0, "translated": 0, "original_only": 0}
...
def main() -> int:
    ...
    translated, cache, stats = translate_records(records, cache, token, args.model, args.batch_size)
